# --MapTraversalAutomatedTest Scene Main Script--
...
# Return: void.
func test_traversal(start_seed: int, end_seed: int) -> void:
	...
	for i in range(start_seed, end_seed):
		...
		if MapTester.test_map_traversal(map_manager.map_structure):
			if i % 1000 == 0:
				print("Status: Success\n")
		else: # If traversal test is unsuccessful execute this branch.
